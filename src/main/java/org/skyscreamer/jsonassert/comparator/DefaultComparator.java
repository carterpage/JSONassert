...
public class DefaultComparator extends AbstractComparator {
    ...
    public void compareValues(String prefix, Object expectedValue, Object actualValue, JSONCompareResult result) throws JSONException {
        if (expectedValue.getClass().isAssignableFrom(actualValue.getClass())) {
            ...
            if (isNull(expectedValue)) {
                result.fail(prefix + ": expected null, but got " + classToType(actualValue));
            } else if (isNull(actualValue)) {
                result.fail(prefix + ": expected " + classToType(expectedValue) + ", but got null");
            } else {
                result.fail("Values of " + prefix + " have different types: expected " + classToType(expectedValue)
    ...
    public void compareJSONArray(String prefix, JSONArray expected, JSONArray actual, JSONCompareResult result) throws JSONException {
        if (expected.length() != actual.length()) {
            result.fail(prefix + "[]: Expected " + expected.length() + " values and got " + actual.length());
            return;
